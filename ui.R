...
#create list of all select options
...
dashboardPage(
  ...
  dashboardSidebar(
    sidebarMenu(
      ...
      menuItem("Martin-Quinn Scores", tabName = 'MQgraph'),
      menuItem('Logistic Regression', tabName = 'issueArea'),
      menuItem('Testing the Model', tabName = 'justice'),
  ...
  dashboardBody(
    tabItems(
      ...
      tabItem(tabName = 'intro',
              box(h1("An Exploration of Supreme Court Voting History"),
                  h4('by Ben Pfeffer'), width = 12),
              box(h2('Welcome!'), 
                  "In this project we will explore the Supreme Court justices’ voting records from a liberal-conservative perspective, using the popular Martin-Quinn score that in order to quantify a justice's ideology. We build a model that quantifies a justice's probability of casting a liberal vote given their Martin-Quinn score and the case's subject area. We then build a second model that simulates the votes of all justices on the court for a given year based on their likelihoods of casting a liberal vote, which we use to predict court outcomes for a given issue area and year and to visualize trends in how the court's decision making has changed over time. I hope you find it enjoyable and illuminating!",
                  width = 12),
              ...
              box(HTML("Click the <b>Data</b> tab to move forward."), width = 12)
      ...
      tabItem(tabName = 'tables',
              fluidRow(
                box(h3("There are two datasets used in this analysis."), 
                    HTML("The analyses in the rest of this app are based on the following two data sets. The Justice by Case data set contains one row for each justice for each court case. The Justice by Year data set contains one row for each justice each year.  You can explore them both below. Explanations for each variable are at the bottom of the page. For the code I used to transform the source data sets into these, see the build-data-set.Rmd file on <a href = https://github.com/ben-pfeffer/supreme-court-shiny> Github</a>."), 
                    width = 12),
                
                #input boxes for filtering
                box(selectInput('dataset', 
                                label = 'Select Data Set', choices = c('Justice by Case',
                                                                       'Justice by Year'))),
                box(
...
HTML("<ul>
<li><b>caseId</b> - the case number from the Supreme Court Database</li>
                                          <li><b>term</b> - the year the court handed down its decision</li>
                                          <li><b>issueArea</b> - one of 14 different categories defined by the Supreme Court Database. We will explore these in more depth later</li>
                                          <li><b>majVotes</b> - the number of justices voting in the majority</li>
                                          <li><b>minVotes</b> - the number of justices voting in the dissent</li>
                                          <li><b>justiceName</b> - the justice's first initial and last name</li>
                                          <li><b>direction</b> - whether the justice cast a conservative or liberal vote. 0 is a conservative vote and 1 is a liberal vote, as determined by the Supreme Court Database. These generally align with the popular understandings of the terms. For details of exactly how decisions are classified <a href = 'http://scdb.wustl.edu/documentation.php?var=decisionDirection'>see here</a></li>
                                          <li><b>majority</b> - whether the justice voted with the majority. 2 is a majority vote and 1 is a dissenting vote </li>
                                          <li><b>MQscore</b> - the justice's Martin-Quinn score. These scores don't mean anything in an absolute sense. The justice's relative positions and the distances between scores are what matter. Positive scores are more conservative, and negative scores are more liberal, but a score of 0 does not automatically represent the exact political center. However, the further a justice's score is from 0, the further that justice's ideology is from the political center. </li>
                                          <li><b>MQrank</b> - the justice's relative rank on the court for the year, as determined by <b>MQscore</b>. A rank of 1 means this is the most liberal member of the court for that year (lowest MQscore). A rank of 9 means this is the most conservative justice for the year (highest MQscore). </li>
                                          <li><b>MQswing.total</b> - the number of swing votes the justice cast in that year. In this project, I  define a swing vote as only occuring in a case decided by 1 vote (usually that means a 5-4 decision). I assign the swing vote to the the most conservative member of a liberal majority, or the most liberal member of a conservative majority, as measured by <b>MQscore</b> <br>
                                          <li><b>case.load</b> - the number of votes the justice cast for the year</li>
                                          <li><b>liberal.vote.total</b> the number of liberal votes a justice cast for the year</li>
                                          <li><b>liberal.vote.pct</b> - the percentage of a justice's votes for the year that were liberal</li></ul>"), width = 12)),
...
<li><b>term</b> - the year the court handed down its decision</li>
                                          ...
                                          <li><b>MQscore</b> - the justice's Martin-Quinn score. These scores don't mean anything in an absolute sense. The justice's relative positions and the distances between scores are what matter. Positive scores are more conservative, and negative scores are more liberal, but a score of 0 does not automatically represent the exact political center. However, the further a justice's score is from 0, the further that justice's ideology is from the political center. </li>
                                          <li><b>MQrank</b> - the justice's relative rank on the court for the year, as determined by <b>MQscore</b>. A rank of 1 means this is the most liberal member of the court for that year (lowest MQscore). A rank of 9 means this is the most conservative justice for the year (highest MQscore). </li>
                                          <li><b>MQswing.total</b> - the number of swing votes the justice cast in that year. In this project, I  define a swing vote as only occuring in a case decided by 1 vote (usually that means a 5-4 decision). I assign the swing vote to the the most conservative member of a liberal majority, or the most liberal member of a conservative majority, as measured by <b>MQscore</b> </li>
                                          <li><b>case.load</b> - the number of votes the justice cast for the year</li>
                                          <li><b>liberal.vote.total</b> the number of liberal votes a justice cast for the year</li>
                                          <li><b>liberal.vote.pct</b> - the percentage of a justice's votes for the year that were liberal</li></ul>"), width = 12))
                )
      ...
      # MARTIN_QUINN SCORES TAB
      tabItem(tabName = 'MQgraph',
              fluidRow(
                box(h3("Martin-Quinn Scores"),
                    HTML("The chart below shows the Martin Quinn score of the median Supreme Court justice from 1946 to 2017. Higher Martin Quinn scores are more conservative, and lower scores are more liberal.<br><br>

                         The exact number of the Martin Quinn score does not mean much on it's own. The relative position or distance between two points is what matters.  The 0 line does not have any special significance (it does not automatically denote the political center), but is included as a useful reference. Pay attention to when and how the Martin Quinn scores move higher and lower, but don't put too much stock in whether they happen to move from one side of the 0 line to the other. The magnitude of the change is much more informative than the sign in front of the number.<br><br>
                         
                         The median justice occupies an important place on the court. There are four justices more liberal and four justices more conservative than the median justice. For this reason, in close decisions - those decided by only one vote - the median justice often determines the outcome of a case. As the median justice becomes more conservative, the court is more likely to cast conservative rulings. When the median justice is more liberal, the court is more likely to cast liberal rulings.  <br><br>
                         "), width = 12),
      ...
      tabItem(tabName = 'issueArea',
              fluidPage(
                box(h3('Likelihood of Voting Liberal'), 
                    HTML("In this project we are interested making a prediction about whether a justice will cast a liberal or a conservative vote depending on their Martin Quinn score. In the chart below each point represents a vote cast by a Supreme Court justice between 1946 and 2017. Points in the top band are liberal votes. Points in the bottom band are conservative votes. The far left side of the chart gives us all the votes by the most liberal justice in our data set. The far right side gives us all the votes by the most conservative. As we move from left to right the justices become more conservative - and more likely to cast a conservative vote."), width = 12),
      ...
      tabItem(tabName = 'justice',
              fluidRow(
                ...
                box(h3("How does the model compare to real life?"),
      ...
      tabItem(tabName = 'court',
              fluidRow(
                box(h3('Predicting Court Outcomes'),
                  ...
                  box(sliderInput('year', label = 'Select a Year', 
      ...
      tabItem(tabName = 'trend',
              fluidRow(
                box(h3("Trends over Time"),
                    "We can use the model predictions for each year to graph the liberal vote probability over time. This also allows us to see which technique (model-based or vote-history based) is making better predictions. For the Economic Activity issue area, the 'Voting History Prediction' seems to have been pretty accurate since around 1980 (the blue and red lines are tracking closely). Note, however, that for one individual year the prediction has not always been close (many of the blue points on the graph are not close to the red line). This is likely an issue of small sample size. The actual results in one given year may not be close to their actual long run average. Check out how the actual percentages over time compare with the models in different issue areas. In what areas has the Supreme Court become more conservative or liberal over time?", width = 12),
